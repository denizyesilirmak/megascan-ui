...
const COLORS = {
  ...
  tunnelJet: [
    ...
    { pct: 255, color: { r: 0x00, g: 0xff, b: 0x00 } }
  ]
}
...
class Plot extends Component {
  ...
  getColor = (pct) => {
    if (this.props.isTunnelScan === true) {
      for (var i = 1; i < COLORS.tunnelJet.length - 1; i++) {
        if (pct < COLORS.tunnelJet[i].pct) {
          break
        }
      }
      const lower = COLORS.tunnelJet[i - 1]
      const upper = COLORS.tunnelJet[i]
