  ...
  render() {
    return (
      <div className="carousel-component">
        <img src={LeftArrow} className={`left-arrow ${(this.props.index !== - 1) ? 'show' : 'hide'}`} alt="la"></img>
        <img src={RightArrow} className={`right-arrow ${(this.props.index !== this.props.buttons.length - 2) ? 'show' : 'hide'}`} alt="la" ></img>
              ...
              this.props.buttons.map((e, k) => {
                return (
                  <div key={k} className={`carousel-button ${this.props.index + 1 === k ? 'selected' : ''}`}>
                    <img alt="ind" className="indicator" src={ButtonIndicator} style={{ display: (this.props.index + 1 === k) ? 'block' : 'none' }}></img>
